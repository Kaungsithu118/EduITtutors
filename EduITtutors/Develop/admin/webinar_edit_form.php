...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $bannerImagePath = $webinar['banner_image']; // Keep existing image by default
        ...
        if (!empty($_FILES['banner_image']['name'])) {
            ...
            if (move_uploaded_file($_FILES['banner_image']['tmp_name'], $targetPath)) {
                ...
                if ($bannerImagePath && file_exists($bannerImagePath)) {
        ...
        $pdo->commit();
        header("Location: webinar_display.php?success=1");
        exit();
    } catch (PDOException $e) {
        $pdo->rollBack();
        die("Error updating webinar: " . $e->getMessage());
    }
}
?>
                ...
                <div class="content-wrapper">
                    ...
                    <div class="row">
                        <div class="col-12 grid-margin">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">Edit Webinar: <?php echo htmlspecialchars($webinar['title']); ?></h4>
                                    ...
                                    <form id="webinar-form" method="POST" enctype="multipart/form-data">
                                        <div class="form-section">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <?php if ($webinar['banner_image']): ?>
                                                    <img id="banner_image_preview" class="preview-image" src="<?php echo $webinar['banner_image']; ?>" alt="Current Banner Image">
                                                <?php else: ?>
                                                    <img id="banner_image_preview" class="preview-image" src="#" alt="Banner Image Preview" style="display: none;">
                                                <?php endif; ?>
                                                <small class="text-muted">Leave blank to keep current image. Recommended size: 1200x400 pixels</small>
                                            </div>

                                            <div class="form-group">
                                                <label for="location" class="required-field">Location</label>
                                                <input type="text" class="form-control" id="location" name="location" value="<?php echo htmlspecialchars($webinar['location']); ?>" required>
                                            </div>

                                            <div class="form-group">
                                                <label for="date" class="required-field">Date</label>
                                                <input type="date" class="form-control" id="date" name="date" value="<?php echo htmlspecialchars($webinar['webinar_date']); ?>" required>
                                            </div>

                                            <div class="form-group">
                                                <label for="time_schedule" class="required-field">Time Schedule</label>
                                                <input type="text" class="form-control" id="time_schedule" name="time_schedule" value="<?php echo htmlspecialchars($webinar['time_schedule']); ?>" placeholder="e.g., 11:00AM to 1:00PM" required>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-section">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" id="intro_text" name="intro_text" rows="4" required><?php echo htmlspecialchars($webinar['intro_text']); ?></textarea>
                                            </div>

                                            <div class="form-group">
                                                <label for="body_text" class="required-field">Main Content</label>
                                                <textarea class="form-control" id="body_text" name="body_text" rows="10" required><?php echo htmlspecialchars($webinar['body_text']); ?></textarea>
                                            </div>

                                            <div class="form-group">
                                                <label for="conclusion_text" class="required-field">Conclusion</label>
                                                <textarea class="form-control" id="conclusion_text" name="conclusion_text" rows="4" required><?php echo htmlspecialchars($webinar['conclusion_text']); ?></textarea>
                                            </div>
                                        </div>
                                        ...
                                        <a href="webinar_display.php" class="btn btn-light">Cancel</a>
                                    </form>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('webinar-form').addEventListener('submit', function(e) {
                ...
                // Basic validation
                const requiredFields = ['title', 'location', 'time_schedule', 'intro_text', 'body_text', 'conclusion_text'];
                let isValid = true;
                ...
                <body>
                    <div class="webinar-header">
                        ...
                        ${document.getElementById('banner_image').files[0] ? 
                          `<img src="${URL.createObjectURL(document.getElementById('banner_image').files[0])}" class="banner-image" alt="Webinar Banner">` : 
                          (document.getElementById('banner_image_preview').style.display !== 'none' ? 
                           `<img src="${document.getElementById('banner_image_preview').src}" class="banner-image" alt="Webinar Banner">` : 
                           '')}
                    </div>
